...
class DocumentEngineeringScreen extends StatefulWidget {
  final EngineeringData? engineeringData;

  const DocumentEngineeringScreen({super.key, this.engineeringData});

  @override
  State<DocumentEngineeringScreen> createState() =>
      _DocumentEngineeringScreenState();
}

class _DocumentEngineeringScreenState extends State<DocumentEngineeringScreen> {
  ...
  Future<bool> willPopScope() {
    Navigator.pop(context, jsonEncode(widget.engineeringData));
              ...
              child: Column(
                children: [
                  ...
                  ListView.builder(
                    ...
                    shrinkWrap: true,
                    itemCount: widget.engineeringData?.documents?.length,
                    itemBuilder: (context, index) {
                      return documentWidget(
                        index: index,
                        engineeringDocument:
                            widget.engineeringData?.documents![index],
                      );
                    },
                  ),
                  const SizedBox(
  ...
  Widget documentWidget({index, Documents? engineeringDocument}) {
    return Padding(
  ...
  _uploadDocuments() async {
    ...
    try {
      ...
      SalesLeadDocumentUploadRes response = await EngineeringRepository()
          .addSalesLeadDocumentApiCall(
              mediaType: selectedMediaType.toString(),
              engineeringID: widget.engineeringData!.id,
              selectedFile: selectedFie);
      if (response.success == true) {
        widget.engineeringData!.documents!.add(
          Documents(
            documentType: selectedMediaType,
            project: widget.engineeringData!.id,
            name: selectedFie!.path.split('/').last,
            attachment: response.attachment,
            created: DateFormat('yyyy-MM-dd').format(DateTime.now()).toString(),
          ),
        );
